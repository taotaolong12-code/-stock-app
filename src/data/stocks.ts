...
export function normalizeStockCode(value:string){const code=value.trim().toLowerCase();if(/^(sh|sz)\d{6}$/.test(code))return code;if(/^6\d{5}$/.test(code))return `sh${code}`;return `sz${code}`;}
function fallback([code,name,pinyin,price]:Seed,index:number):StockItem{const prevClose=Number((price*(1-((index%7)-3)*.012)).toFixed(2));const change=Number((price-prevClose).toFixed(2));return {code,name,pinyin,price,prevClose,change,percent:Number((change/prevClose*100).toFixed(2)),volume:0,high:price,low:price,pe:0,pb:0,eps:0,roe:0,turnover:0};}
...
export async function fetchStockCandles(code:string,period:PeriodKey){const symbol=normalizeStockCode(code);const param=periodParam[period];const minute=period.endsWith('m');try{const json=JSON.parse(await request(`https://web.ifzq.gtimg.cn/appstock/app/kline/${minute?'mkline':'kline'}?code=${symbol}&param=${param}`));const raw=json?.data?.[symbol]?.[param]||json?.data?.[symbol]?.data||json?.data?.[symbol]?.[period]||[];const points=(Array.isArray(raw)?raw:[]).map((row:any,index:number)=>({time:String(row?.[0]??index),close:Number(row?.[4]??row?.[2]??row?.close)})).filter((point:{close:number})=>point.close>0);if(points.length)return points.slice(-240);}catch{/* fallback below */}const stock=(await fetchStockQuotes([symbol]))[0]||buildMarketStocks()[0];return buildHistoricalSeries(stock,period);}
export function searchStocks(query:string,stocks=buildMarketStocks()){const value=query.trim().toLowerCase();return value?stocks.filter(stock=>stock.code.includes(value)||stock.name.includes(value)||stock.pinyin.includes(value)).slice(0,20):stocks.slice(0,10);}
export function buildHistoricalSeries(stock:StockItem,period:PeriodKey){const length=period==='1m'?32:period.endsWith('m')?60:period==='year'?12:80;return Array.from({length},(_,index)=>({time:String(index),close:Number((stock.prevClose+(stock.price-stock.prevClose)*index/Math.max(length-1,1)).toFixed(2))}));}
export function computeIndicatorSummary(series:Array<{close:number}>){const closes=series.map(point=>point.close),last=closes.at(-1)||0,previous=closes.at(-2)||last,average=closes.reduce((sum,value)=>sum+value,0)/Math.max(closes.length,1),macd=Number((last-average).toFixed(2)),kdj=Number(((last-previous)*1.8+average*.2).toFixed(2)),rsi=Number((last/Math.max(previous,1)*100).toFixed(2));return {macd,kdj,rsi,buySignals:macd>0?['MACD金叉']:[],sellSignals:macd<0?['MACD死叉']:[],pattern:macd>0?'上升通道':macd<0?'下跌通道':'震荡盘整'};}
